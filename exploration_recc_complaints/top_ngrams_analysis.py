...
def str_with_prepared_text(data:pd.DataFrame, text_process:str="normal", filter_var:str=None)->str:
    """
...
def prepare_data_for_wordcloud(data:pd.DataFrame, generate_from_text:bool=True, text_process:str="normal", filter_var:str=None) ->str|pd.DataFrame:
    """
    ...
    if generate_from_text:
        data_ready_for_wordcloud =  str_with_prepared_text(data, text_process, filter_var)
    else:
        data_ready_for_wordcloud = compute_tf_idf_dataframe(data, text_process, filter_var)
...
if __name__ == "__main__":
    ...
    for variant in top_ngrams_variants.keys():
        ...
        if "filter" in variant_params:
            ...
            data_ready_for_wordcloud = prepare_data_for_wordcloud(recc_data, generate_from_text, text_process, filter_var = filter)
        else:
            data_ready_for_wordcloud = prepare_data_for_wordcloud(recc_data, generate_from_text, text_process)
        
        visualisation_utils.wordcloud(data_ready_for_wordcloud, stopwords=stopwords_list, variant_name = variant, generate_from_text=generate_from_text, max_words=max_words)