...
def keyword_expression_dummies(data:pd.DataFrame):
    for g in keywords_expressions.keys():
        for ke in keywords_expressions[g]:
            if " " in ke:
                data["EXP:"+ke] = data["processed_complaint_summary"].apply(lambda x: 1 if ke in x else 0)
            else:
...
def group_keyword_expression_dummies(data):
    for g in keywords_expressions.keys():
        ...
        columns = columns + ["EXP:"+col for col in keywords_expressions[g] if " " in col]
...
if __name__ == "__main__":
    ...
    dummies_keywords_expressions = [col for col in recc_data.columns if (col.startswith("KEYW:") or col.startswith("EXP:"))]
    stats_keywords_expressions = processing_utils.complaints_by(df = recc_data, by = dummies_keywords_expressions, dummy_vars=True, percent=True, sort=True)
    stats_keywords_expressions = stats_keywords_expressions.loc[:10]
    visualisation_utils.plotting_complaints_by_dummies(stats_keywords_expressions, "keywords and expressions (top 10)")
    ...
    stats_groups = processing_utils.complaints_by(df = recc_data, by = dummies_groups, dummy_vars=True, percent=True, sort=True)
